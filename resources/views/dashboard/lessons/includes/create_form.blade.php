...
<form method="POST"
      action="{{ route($routeNames->store) }}"
      class="mb-3">
    @csrf
    @include('dashboard.lessons.includes.fields.multi_date_mode')
    <div class="row">
        <div class="@if($user->isCreator()){!!'col-12'!!}@else{!!'col-8'!!}@endif">
            <div class="row mb-3">
                <div class="col">
            ...
            <div class="row mb-3">
                <div class="col-auto">
                    @include('dashboard.lessons.includes.fields.date_with_check', ['date' => $lesson->getDefaultDate()])
                </div>
            ...
            <div class="row mb-3">
                <div class="col-auto">
                    @include('dashboard.lessons.includes.fields.timeslots_new')
                </div>
                @can($abilities->showAdminOption())
                    <div class="col-auto">
                        @include('dashboard.lessons.includes.fields.room_id', ['roomId' => null])
                    </div>
                @endcan
            </div>
            <div class="row mb-3">
        ...
        @can($abilities->showAdminOption())
            <div class="col-4 mt-4">
                @include('dashboard.lessons.includes.manager_area', [
                    'status'        => $lesson->getDefaultStatus(),
                    'adminFeedback' => null
                ])
